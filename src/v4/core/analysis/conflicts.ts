...
import { mapPropertyToTheme } from '../extraction/rules';
...
export function detectConflicts(
  ...
  for (const rule of cssRules) {
    ...
    const themeNamespace = theme[mapping.themeProperty];
    if (!isRecord(themeNamespace)) {
      continue;
    }

    const themeValue = themeNamespace[themeKey];
    if (!isString(themeValue)) {
      continue;
    }
...
export function applyOverride(theme: Theme, conflict: CSSRuleConflict): void {
  const namespace = theme[conflict.themeProperty];
