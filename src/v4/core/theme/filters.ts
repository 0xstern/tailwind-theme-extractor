...
import { NAMESPACE_TO_THEME_PROPERTY } from '../constants/namespaces';
import { parseVariableName } from '../parser/extractor';
...
export function filterThemeByExclusions(
  ...
  // Identify which properties need to be copied based on exclusions
  const affectedProperties = new Set<keyof Theme>();
  for (const namespace of exclusionsByNamespace.keys()) {
    const property = NAMESPACE_TO_THEME_PROPERTY[namespace];
    if (property !== undefined) {
      affectedProperties.add(property);
    }
  }

  // Create a shallow copy of the theme first
  const filteredTheme: Theme = { ...theme };

  // Only deep copy affected properties
  for (const property of affectedProperties) {
    const namespaceExclusions = exclusionsByNamespace.get(
      getNamespaceForProperty(property),
    );
    if (namespaceExclusions === undefined) {
      continue;
    }
    ...
    const themeProperty = { ...(theme[property] as Record<string, unknown>) };
...
/**
 * Gets the namespace for a given theme property
 * @param property - Theme property key
 * @returns Namespace string
 */
function getNamespaceForProperty(property: keyof Theme): string {
  // Create reverse mapping from NAMESPACE_TO_THEME_PROPERTY
  for (const [namespace, prop] of Object.entries(NAMESPACE_TO_THEME_PROPERTY)) {
    if (prop === property) {
      return namespace;
    }
  }
...
/**
 * Strategy function for filtering property values
 * Returns the filtered value or null if it should be excluded
 */
type FilterStrategy<TValue> = (
  key: string,
  value: TValue,
  exclusions: Array<InitialExclusion>,
  namespace: string,
...
/**
 * Generic property filter using a filtering strategy
 *
 * @param property - Property object to filter
 * @param exclusions - Exclusion patterns to apply
 * @param namespace - CSS variable namespace
 * @param strategy - Strategy function for filtering values
 * @returns Filtered property object
 */
function filterPropertyWithStrategy<TValue>(
  property: Record<string, TValue>,
  exclusions: Array<InitialExclusion>,
  namespace: string,
  strategy: FilterStrategy<TValue>,
): Record<string, TValue> {
  const filtered: Record<string, TValue> = {};

  for (const [key, value] of Object.entries(property)) {
    const result = strategy(key, value, exclusions, namespace);
    if (result !== null) {
      filtered[key] = result;
    }
  }
...
/**
 * Strategy for filtering simple string values
 *
 * @param key - Property key
 * @param value - Property value
 * @param exclusions - Exclusion patterns to check
 * @param namespace - CSS variable namespace
 * @returns Filtered value or null if excluded
 */
function simpleValueStrategy(
  ...
  return shouldExclude ? null : value;
}
...
function filterColors(
  ...
  for (const [colorName, colorValue] of Object.entries(colors)) {
    ...
    if (typeof colorValue === 'string') {
      // Flat color - use simple strategy
      const result = simpleValueStrategy(
        colorName,
        colorValue,
        exclusions,
        'color',
      ...
      if (result !== null) {
        filtered[colorName] = result as string;
      }
    } else {
...
function filterFontSizes(
  ...
  return filterPropertyWithStrategy(
    fontSize,
    exclusions,
    'text',
    simpleValueStrategy,
...
export function applyInitialExclusionToTheme(
  ...
  const property = NAMESPACE_TO_THEME_PROPERTY[exclusion.namespace];
  if (property === undefined) {
    return;
  }
