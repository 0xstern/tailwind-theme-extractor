 ...
 * Sized to handle typical override configurations
 */
 ...
 */
const flatPathCache = new LRUCache<string, Array<string>>(MAX_CACHE_SIZE);

/**
 * Cache for memoizing variable name generation (e.g., ['colors', 'primary'] → '--colors-primary')
 * Uses LRU eviction to prevent unbounded growth in long-running processes
 */
const variableNameCache = new LRUCache<string, string | null>(MAX_CACHE_SIZE);
