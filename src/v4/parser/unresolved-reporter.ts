...
import type {
  UnresolvedCause,
  UnresolvedVariable,
} from './unresolved-detector';

import { mkdir, writeFile } from 'node:fs/promises';
import { join } from 'node:path';

import { groupByLikelyCause } from './unresolved-detector';
 ...
 */
interface UnresolvedReportJSON {
  generatedAt: string;
  source: string;
  version?: string;
...
/**
 * Indentation spaces for JSON formatting
 */
...
export async function writeUnresolvedReports(
  ...
  // Write files
  const markdownPath = join(outputDir, 'unresolved.md');
  const jsonPath = join(outputDir, 'unresolved.json');
