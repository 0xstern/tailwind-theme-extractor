...
import {
  ...
  mergeThemes,
} from '../../../src/v4/core/theme/defaults';

describe('Tailwind Default Theme', () => {
  test('loads Tailwind defaults when includeDefaults is true', async () => {
  ...
  test('does not load defaults when includeDefaults is false', async () => {
    const result = await resolveTheme({
      css: '@theme { --color-primary-500: #custom; }',
...
describe('loadTailwindDefaults - Direct function tests', () => {
  test('returns null or result object with theme and variables', async () => {
    const result = await loadTailwindDefaults(process.cwd());

    // Result should be either null (not installed) or a valid result object
    if (result === null) {
  ...
  test('returns valid theme structure when Tailwind is installed', async () => {
    ...
    expect(typeof result.theme.colors).toBe('object');
    expect(typeof result.theme.spacing).toBe('object');
...
describe('loadTailwindDefaults - Cache behavior', () => {
  ...
  test('loads theme when no cache exists (cold start)', async () => {
    ...
    // With cleared cache, this hits the no-cache path
    const result = await loadTailwindDefaults(basePath);

    // Should either return null (not installed) or result object
    if (result === null) {
      expect(result).toBeNull();
    } else {
      expect(result).toHaveProperty('theme');
...
describe('Nesting configuration on Tailwind defaults', () => {
  test('applies maxDepth to Tailwind default fontSize variables', async () => {
    const result = await resolveTheme({
      css: '@theme { --color-custom: blue; }',
      includeDefaults: { fontSize: true },
      nesting: {
        fontSize: { maxDepth: 0, flattenMode: 'literal' },
      },
    });

    // Skip if Tailwind not installed
    if (result.variants.default.fontSize.xl === undefined) {
      expect(result.variants.default.fontSize.xl).toBeUndefined();
      return;
    }

    // With maxDepth: 0 and flattenMode: 'literal', Tailwind default --text-xl should become fontSize['xl']
    // Not fontSize.xl which would be the normal structure
    expect(result.variants.default.fontSize.xl).toBeDefined();
    expect(typeof result.variants.default.fontSize.xl).toBe('object');
  });

  test('applies consecutiveDashes config to both user and default variables', async () => {
    const result = await resolveTheme({
      css: '@theme { --color-button--primary: red; }',
      includeDefaults: { colors: true },
      nesting: {
        colors: { consecutiveDashes: 'camelcase' },
      },
    });

    // User variable with consecutive dashes should use camelCase
    expect(result.variants.default.colors.buttonPrimary).toBe('red');

    // If Tailwind has any default variables with consecutive dashes,
    // they should also follow the same rule (though Tailwind typically doesn't use them)
    // This test ensures config is applied uniformly
  });

  test('applies flattenMode to Tailwind defaults', async () => {
    const result = await resolveTheme({
      css: '@theme { --color-custom: blue; }',
      includeDefaults: true,
      nesting: {
        default: { maxDepth: 1, flattenMode: 'literal' },
      },
    });

    // Skip if Tailwind not installed or no nested defaults exist
    if (result.variants.default.colors.red === undefined) {
      expect(result.variants.default.colors.red).toBeUndefined();
      return;
    }

    // Tailwind default colors like --color-red-500 should be flattened to colors.red['500']
    // NOT colors.red[500] with numeric key
    // With maxDepth: 1, would be colors['red-500']
    const red = result.variants.default.colors.red;
    if (typeof red !== 'string') {
      // Should still have color scale structure for colors (maxDepth defaults to Infinity for colors namespace when not overridden)
      expect(red).toBeDefined();
    }
  });

  test('Tailwind defaults respect per-namespace nesting config', async () => {
    const result = await resolveTheme({
      css: '@theme {}',
      includeDefaults: { colors: true, fontSize: true, spacing: true },
      nesting: {
        colors: { maxDepth: 2 },
        fontSize: { maxDepth: 0, flattenMode: 'literal' },
        spacing: { maxDepth: 1 },
    ...
    // Colors: maxDepth 2 - should maintain nested structure for two levels
    const red = result.variants.default.colors.red;
    if (typeof red !== 'string') {
      expect(red).toBeDefined();
      // Should have numeric keys like 500, 600
      expect(red[500] ?? red['500']).toBeDefined();
    }

    // FontSize: maxDepth 0 - should flatten all fontSize defaults
    expect(result.variants.default.fontSize.xl).toBeDefined();
  });

  test('caches Tailwind defaults separately for different nesting configs', async () => {
    clearDefaultThemeCache();

    // Load with maxDepth: 0
    const result1 = await resolveTheme({
      css: '@theme {}',
      includeDefaults: { colors: true },
      nesting: {
        colors: { maxDepth: 0, flattenMode: 'literal' },
      },
    });

    // Load with maxDepth: 2
    const result2 = await resolveTheme({
      css: '@theme {}',
      includeDefaults: { colors: true },
      nesting: {
        colors: { maxDepth: 2 },
    ...
    // Skip if Tailwind not installed
    if (
      result1.variants.default.colors.red === undefined &&
      result2.variants.default.colors.red === undefined
    ) {
      expect(result1.variants.default.colors.red).toBeUndefined();
      expect(result2.variants.default.colors.red).toBeUndefined();
      return;
    }

    // Results should be different due to different nesting configs
    // With maxDepth: 0, colors are flat (e.g., colors['red-500'])
    // With maxDepth: 2, colors are nested (e.g., colors.red[500])
    const keys1 = Object.keys(result1.variants.default.colors);
    const keys2 = Object.keys(result2.variants.default.colors);

    // Structure should differ based on nesting config
    // maxDepth: 0 produces flat keys like 'red-500', maxDepth: 2 produces nested like 'red'
  ...
  test('user variables and defaults have consistent nesting behavior', async () => {
    const result = await resolveTheme({
      css: '@theme { --text-custom-large: 2rem; }',
      includeDefaults: { fontSize: true },
      nesting: {
        fontSize: { maxDepth: 0, flattenMode: 'literal' },
      },
    ...
    // User variable
    expect(result.variants.default.fontSize['custom-large']).toBeDefined();
    expect(result.variants.default.fontSize['custom-large']!.size).toBe('2rem');

    // Tailwind default (if exists)
    if (result.variants.default.fontSize.xl !== undefined) {
      // Should also use literal flat structure, not nested
      expect(result.variants.default.fontSize.xl).toBeDefined();
    }
  });

  test('nesting config applies to defaults loaded from node_modules', async () => {
    // This test verifies the complete integration: loadTailwindDefaults receives nesting config
    const result = await resolveTheme({
      css: '@theme {}',
      includeDefaults: true,
      nesting: {
    ...
    // Skip if Tailwind not installed
    if (Object.keys(result.variants.default.colors).length === 0) {
      expect(Object.keys(result.variants.default.colors).length).toBe(0);
      return;
    }

    // With maxDepth: 1, all Tailwind colors should be flattened
    // e.g., --color-red-500 becomes colors.red500 (not colors.red[500])
    // Verify that defaults were loaded and processed
    expect(result.variants.default).toBeDefined();
  });
});

describe('Granular includeDefaults - Integration tests', () => {
  ...
  test('excluding all categories is equivalent to includeDefaults: false', async () => {
