...
import { isConflictReportJSON } from '../../../src/v4/core/reporting/conflicts';
    ...
    try {
      ...
      if (
        ...
        const { writeConflictReports } = await import(
          '../../../src/v4/core/reporting/conflicts'
        );
        ...
        const parsedData = parseJSON(jsonContent, isConflictReportJSON);

        // Now we have type-safe access
        expect(parsedData.summary).toBeDefined();
        expect(parsedData.conflicts).toBeDefined();
        expect(Array.isArray(parsedData.conflicts)).toBe(true);
