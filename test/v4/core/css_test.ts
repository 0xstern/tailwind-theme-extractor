...
describe('parseCSS - File input parsing', () => {
  test('parses CSS from file path', async () => {
    const result = await parseCSS({
      input: join(FIXTURES_DIR, 'base_theme.css'),
    });

    expect(result.theme.colors).toBeDefined();
    expect(result.files).toContain(join(FIXTURES_DIR, 'base_theme.css'));
  });

  test('returns absolute file paths in files array', async () => {
    const inputPath = join(FIXTURES_DIR, 'base_theme.css');
    const result = await parseCSS({
      input: inputPath,
...
describe('parseCSS - Import resolution', () => {
  test('resolves nested imports', async () => {
    const result = await parseCSS({
      input: join(FIXTURES_DIR, 'imports_level1.css'),
    });

    // Should include all three levels
    expect(result.files).toContain(join(FIXTURES_DIR, 'imports_level1.css'));
    expect(result.files).toContain(join(FIXTURES_DIR, 'imports_level2.css'));
    expect(result.files).toContain(join(FIXTURES_DIR, 'imports_level3.css'));
  });
