...
import type { CSSRuleConflict } from '../../../src/v4/core/analysis/conflicts';
...
import {
  ...
  isConflictReportJSON,
} from '../../../src/v4/core/reporting/conflicts';
...
describe('JSON Report Generation', () => {
  test('generates valid JSON with metadata', () => {
    ...
    const parsed = parseJSON(json, isConflictReportJSON);
  ...
  test('includes media query info when present', () => {
    ...
    const firstConflict = parsed.conflicts[0];
    if (firstConflict === undefined) {
      throw new Error('Expected first conflict to exist');
    }
    expect(firstConflict.inMediaQuery).toBe(true);
    expect(firstConflict.mediaQuery).toBe('(min-width: 1024px)');
  });
  ...
  test('marks applied status correctly', () => {
    ...
    const firstConflict = parsed.conflicts[0];
    const secondConflict = parsed.conflicts[1];
    if (firstConflict === undefined || secondConflict === undefined) {
      throw new Error('Expected two conflicts to exist');
    }
    expect(firstConflict.applied).toBe(true); // high confidence
    expect(secondConflict.applied).toBe(false); // medium confidence
  });
});
