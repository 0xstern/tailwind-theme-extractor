...
describe('CSS Conflict Detection', () => {
  test('detects and reports CSS conflicts when present', () => {
    // The fixture may or may not have conflicts, so this is informational
    if (
      result.cssConflicts !== undefined &&
      Array.isArray(result.cssConflicts)
    ) {
