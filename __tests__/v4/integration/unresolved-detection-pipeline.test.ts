    ...
    try {
      ...
      if (
        ...
        const { writeUnresolvedReports } = await import(
          '../../../src/v4/parser/unresolved-reporter'
        );
        ...
        expect(jsonData.summary.total).toBeGreaterThan(0);
        expect(jsonData.unresolved).toBeDefined();
        expect(Array.isArray(jsonData.unresolved)).toBe(true);
