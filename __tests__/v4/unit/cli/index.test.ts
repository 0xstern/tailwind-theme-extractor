...
describe('CLI - Granular Tailwind Defaults', () => {
  ...
  it('should test granular defaults using mergeThemes with fixture', async () => {
    const { mergeThemes } = await import(
      '../../../../src/v4/parser/tailwind-defaults'
    );
    const { resolveTheme } = await import('../../../../src/v4/index');

    const fixtureDir = join(process.cwd(), '__tests__/v4/fixtures');
    const baseThemePath = join(fixtureDir, 'base-theme.css');
